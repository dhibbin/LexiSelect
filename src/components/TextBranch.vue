<template>
  <v-list class="horizontal-list bg-background" style="min-width: max-content;" @mouseleave="setExpand(false, 0.5)">
    <v-list-item v-for="(token, index) in tokens" :key="index" :value="token"
      :class="'pa-0 ' + (props.isActive ? 'bg-primary' : 'bg-background')" style="overflow: visible;"
      @click="click(index, $event)" @mouseenter="setMenuHeight(index)">
    ...
    <v-card v-show="expand" ref="tokenMenu" :style="{
      ...
      <v-list class="pa-0" dense="true">
        ...
        <v-list-item
          ...
          :key="probIndex" class="pa-0 ma-0" :value="tokenProb"
          :disabled="tokenProb.tok_str == tokens[currTokIndex].completionProb.content" density="compact"
          @click="newBranch(currTokIndex, tokenProb.tok_str)">
          <v-list-item-title style="font-family: monospace; text-align: center;" class="pa-1">
            {{ tokenProb.tok_str }}
          </v-list-item-title>
          <template #prepend>
            <v-sheet class="pa-1 ma-1 v-align-center v-text-center" style="width: 58px; text-align: center;"
              color="info" rounded="sm">
  ...
  <v-card :style="{
    position: 'absolute', top: 0 + 'px', fontFamily: 'monospace',
    left: 0 + 'px', height: 'min-content'
  }" class="mx-auto hidden" width="200">
    <v-list class="pa-0" dense="true">
      <v-text-field ref="exampleTextField" hide-details />
      <v-list-item class="pa-0 ma-0" density="compact">
        <v-list-item-title ref="exampleTokenListItem" style="font-family: monospace; text-align: center;" class="pa-1">
          {{ "Example" }}
        </v-list-item-title>
        <template #prepend>
          <v-sheet class="pa-1 ma-1 v-align-center v-text-center" style="width: 58px; text-align: center;" color="info"
            rounded="sm">
            {{ "100%" }}
          </v-sheet>
        </template>
